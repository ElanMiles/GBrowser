...
from app.animations import slide_geometry
...
class _TabButton(QWidget):
    ...
    _STYLE_INACTIVE = (
        "QWidget#tab_btn_container { background: transparent; border-radius: 8px; border: none; }"
...
class TabPanel(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self.indicator = QWidget(self.container)
        self.indicator.setStyleSheet(
            "background: qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #5a6eff, stop:1 #9a5aff);"
            "border-radius: 2px;"
        )
    ...
    def set_current_index(self, index: int) -> None:
        ...
        self._move_indicator(index, animate=self._indicator_ready)
    ...
    def _move_indicator(self, index: int, animate: bool = True) -> None:
        ...
        target_rect = QRect(
            target_btn.x() + 6,
            self.container.height() - 3,
            max(target_btn.width() - 12, 10),
            3,
        )

        if animate and self.indicator.isVisible() and not self.indicator.geometry().isEmpty():
            slide_geometry(self.indicator, target_rect, duration=180)
        else:
