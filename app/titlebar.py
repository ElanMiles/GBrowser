    ...
    def mousePressEvent(self, event) -> None:
        if event.button() == Qt.LeftButton:
            ...
            if win is not None and win.isMaximized():
                # Сохраняем относительную позицию курсора в заголовке — для корректного
                # восстановления позиции окна при перетаскивании из максимизированного состояния.
                rel_x = event.position().x() / max(1.0, self.width())
                self._drag_offset = ("max", rel_x)
            elif win is not None:
                # Обычное поведение: запоминаем смещение между глобальной позицией и верхним левым углом окна
