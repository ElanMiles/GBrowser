...
from app.discord_rpc_manager import DiscordPresenceManager, DEFAULT_CLIENT_ID
...
class AcrylicBackgroundBrowser(QWidget):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("GBrowser Alpha 3")
        self.setWindowFlags(Qt.WindowType.FramelessWindowHint)
        self.setAttribute(Qt.WidgetAttribute.WA_NoSystemBackground, False)
        self.setStyleSheet("AcrylicBackgroundBrowser { background-color: #0d0d1a; }")
        ...
        self._backdrop_mode: str = self._q_settings.value("backdrop_mode", "Acrylic", type=str)
        self._discord_enabled: bool = self._q_settings.value("discord_enabled", False, type=bool)
        self._discord_client_id: str = self._q_settings.value("discord_client_id", "", type=str)
        ...
        self._backdrop_active: bool = False
        self._applied_backdrop_key: Optional[tuple] = None
        ...
        frame.setStyleSheet(
            "QFrame#web_frame { background: #0d0d1a; border-radius: 12px; }"
        )
        ...
        self._web_frame = frame

        self.discord = DiscordPresenceManager(self)
        if self._discord_enabled:
            self.discord.enable(self._discord_client_id or DEFAULT_CLIENT_ID)

        self._status_timer = QTimer(self)
        self._status_timer.setInterval(4000)
        self._status_timer.timeout.connect(self._refresh_discord_indicator)
    ...
    def _apply_acrylic(self, force: bool = False) -> None:
        key = (self._system_transparency, self._backdrop_mode)
        if not force and key == self._applied_backdrop_key:
            return
        self._applied_backdrop_key = key

        if self._system_transparency and effects.supports_modern_backdrop():
            self.setAttribute(Qt.WidgetAttribute.WA_NoSystemBackground, True)
            try:
                self._backdrop_active = effects.apply_system_backdrop(self, self._backdrop_mode)
            except Exception:
                logger.exception("Failed to apply backdrop effect")
                self._backdrop_active = False
        else:
            self.setAttribute(Qt.WidgetAttribute.WA_NoSystemBackground, False)
            self._backdrop_active = False
            try:
                effects.remove_system_backdrop(self)
            except Exception:
                logger.exception("Failed to remove backdrop")
    ...
    def _exit_fullscreen(self) -> None:
        ...
        if self._was_maximized:
            ...
            self.showNormal()
        self._apply_acrylic(force=True)
        self.titlebar.update_maximize_icon(self.isMaximized())
    ...
    def _on_current_changed(self, index: int) -> None:
        ...
        try:
            ...
            self.titlebar.back.setEnabled(view.history().canGoBack())
            self.titlebar.fwd.setEnabled(view.history().canGoForward())
        except Exception:
            pass
    ...
    def _update_discord_presence(self) -> None:
        ...
        try:
            view = self.tabs.current_view()
            title = self.titlebar.title.text() or "GBrowser"
            url = view.url().toString()
            self.discord.update_browsing(title, url)
        except Exception:
            pass

    def _refresh_discord_indicator(self) -> None:
        try:
            self.titlebar.set_discord_status(self._discord_enabled and self.discord.is_connected())
        except Exception:
            pass
    ...
    def open_settings(self) -> None:
        dialog = SettingsDialog(
            ...
            initial_backdrop_mode=self._backdrop_mode,
            initial_discord_enabled=self._discord_enabled,
            initial_discord_client_id=self._discord_client_id,
        )
    ...
    def apply_settings(self, s: dict) -> None:
        ...
        self._backdrop_mode = s["backdrop_mode"]
        ...
        self._q_settings.setValue("backdrop_mode", self._backdrop_mode)
        self._q_settings.setValue("discord_enabled", discord_enabled)
        self._q_settings.setValue("discord_client_id", discord_client_id)

        if discord_enabled != self._discord_enabled or discord_client_id != self._discord_client_id:
            self._discord_enabled = discord_enabled
            self._discord_client_id = discord_client_id
            if self._discord_enabled:
                self.discord.enable(self._discord_client_id or DEFAULT_CLIENT_ID)
            else:
                self.discord.disable()
