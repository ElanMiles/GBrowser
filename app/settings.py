...
class SettingsDialog(QDialog):
    ...
    def __init__(
        ...
        initial_backdrop_mode: str = "Acrylic",
        initial_discord_enabled: bool = False,
        initial_discord_client_id: str = "",
    ) -> None:
        ...
        backdrop_lbl = QLabel("Backdrop material:")
        backdrop_lbl.setStyleSheet(lbl_style)
        self.backdrop_combo = QComboBox()
        self.backdrop_combo.addItems(["Acrylic", "Mica"])
        self.backdrop_combo.setCurrentText(initial_backdrop_mode)
        backdrop_row.addWidget(backdrop_lbl)
        backdrop_row.addWidget(self.backdrop_combo)
        ...
        discord_sep = QFrame()
        discord_sep.setFrameShape(QFrame.Shape.HLine)
        discord_sep.setStyleSheet("color: rgba(255,255,255,0.1);")
        layout.addWidget(discord_sep)

        self.discord_checkbox = QCheckBox("Enable Discord Rich Presence")
        self.discord_checkbox.setStyleSheet(lbl_style)
        self.discord_checkbox.setChecked(initial_discord_enabled)
        layout.addWidget(self.discord_checkbox)

        discord_row = QHBoxLayout()
        discord_lbl = QLabel("Discord client ID:")
        discord_lbl.setStyleSheet(lbl_style)
        self.discord_client_edit = QLineEdit()
        self.discord_client_edit.setPlaceholderText("Optional, uses default GBrowser application")
        self.discord_client_edit.setText(initial_discord_client_id)
        self.discord_client_edit.setStyleSheet(
            "QLineEdit { background:rgba(255,255,255,0.08); color:white;"
            "border:1px solid rgba(255,255,255,0.15); border-radius:6px; padding:4px 10px; }"
        )
        discord_row.addWidget(discord_lbl)
        discord_row.addWidget(self.discord_client_edit, 1)
    ...
    def _on_save(self) -> None:
        ...
        s = {
            ...
            "backdrop_mode": self.backdrop_combo.currentText(),
            "discord_enabled": self.discord_checkbox.isChecked(),
            "discord_client_id": self.discord_client_edit.text().strip(),
        }
